...
public class ErasureCodeBenchmarkThroughput extends Configured implements Tool {
  ...
  private static final String REP_DIR = DFS_TMP_DIR + "/replica";
  private static final String EC_DIR = DFS_TMP_DIR + "/ec";
  ...
  private static String getFilePath(int dataSizeMB, boolean isEc) {
  ...
  private List<Long> doBenchmark(boolean isRead, int dataSizeMB, int numClients,
    ...
    for (int i = 0; i < numClients; i++) {
      futures.add(executor.submit(isRead ?
  ...
  private void setUpDir() throws IOException {
    ...
    if (!dfs.exists(ecPath)) {
      dfs.mkdirs(ecPath);
      dfs.getClient().setErasureCodingPolicy(ecPath.toString(), null);
    } else {
      Preconditions.checkNotNull(
          dfs.getClient().getErasureCodingPolicy(ecPath.toString()));
    }
  }
  ...
  private class WriteCallable extends CallableBase {
    ...
    private long writeFile(Path path) throws IOException {
      ...
      try (FSDataOutputStream outputStream = fs.create(path)) {
        ...
        int bufSize = BUFFER_SIZE_MB * 1024 * 1024;
        int toWrite;
        byte buf[] = new byte[bufSize];
        while (remaining > 0) {
          random.nextBytes(buf);
          toWrite = (int) Math.min(remaining, bufSize);
