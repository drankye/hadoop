...
#include "org_apache_hadoop_io_erasurecode_rawcoder_XORRawDecoder.h"
#include <jni.h>
#include <pthread.h>
#include <signal.h>
#include <dlfcn.h>
...
JNIEXPORT jint JNICALL Java_org_apache_hadoop_io_erasurecode_rawcoder_ISARSRawDecoder_init(
  ...
  if(NULL == (pCodecParameter = (Codec_Parameter *)pthread_getspecific(de_key))){
    ...
    dlsym_ec_init_tables(stripeSize, paritySize,
      &(pCodecParameter->a)[stripeSize * stripeSize], pCodecParameter->g_tbls);
...
JNIEXPORT jint JNICALL Java_org_apache_hadoop_io_erasurecode_rawcoder_ISARSRawDecoder_decode
  ...
  for(j = 0; j < erasureLen; j++){
    if (tmp[j] >= pCodecParameter->paritySize) {
      // errors in parity will not be fixed
      errorlocation = tmp[j] - pCodecParameter->paritySize;
      pCodecParameter->erasured[errorlocation] = 1;
    } else if (tmp[j] >= 0) { // put error parity position
  ...
  for(j = 0, r = 0; j < pCodecParameter->paritySize +
                                          pCodecParameter->stripeSize; j++ ) {
    if(pCodecParameter->erasured[j] == 1) {
      src_error_list[r++] = j ;
    }
  }

  for(j = pCodecParameter->paritySize, r = 0, i = 0;
          j < pCodecParameter->paritySize + pCodecParameter->stripeSize; j++) {
    pCodecParameter->datajbuf[j] = (*env)->GetObjectArrayElement(env, alldata, j);
    pCodecParameter->data[j] = (u8 *)(*env)->GetDirectBufferAddress(env,
                  pCodecParameter->datajbuf[j]);
  ...
  if(gf_invert_matrix(pCodecParameter->b, pCodecParameter->d,
  pCodecParameter->stripeSize) < 0){
  ...
  for(i = srcErrors, p = 0; i < erasureLen; i++, p++) {
    for(j = 0; j < pCodecParameter->stripeSize; j++)  {
      pCodecParameter->e[pCodecParameter->stripeSize * p + j] =
      pCodecParameter->a[pCodecParameter->stripeSize * src_error_list[i] + j];
    }
  }
  ...
  for(p = 0; p < parityErrors; p++) {
    for(i = 0; i < pCodecParameter->stripeSize; i++) {
      s = 0;
      for(j = 0; j < pCodecParameter->stripeSize; j++) {
        s ^= gf_mul(pCodecParameter->d[j * pCodecParameter->stripeSize + i],
        pCodecParameter->e[pCodecParameter->stripeSize * p + j]);
      }
