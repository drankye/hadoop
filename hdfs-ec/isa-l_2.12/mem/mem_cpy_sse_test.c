...
int main(int argc, char *argv[])
{
	...
	printf("mem_cpy_sse_test %d bytes, %d randoms, seed=0x%x\n",
	...
	mem_cpy_sse(des, src, TEST_LEN);
	if (memcmp(des, src, TEST_LEN)) {
		printf("Error: whole length test failed - mem_cpy_sse\n");
		ret |= 1;
	} else {
		putchar('.');
		fflush(0);
	...
	for (i = 0; i < RANDOMS; i++) {
		...
		*(src + r) = *(des + r) + 1;
		mem_cpy_sse(des, src, TEST_LEN);
		if (memcmp(des, src, TEST_LEN)) {
			printf("Error: random pos test failed (r: %d) - mem_cpy_sse\n", r);
			ret |= 1;
		} else {
			putchar('.');
			fflush(0);
	...
	for (i = 0; i < RANDOMS; i++) {
		...
		memset(src, r, r);
		memset(des, r * 3 + 1, r);
		mem_cpy_sse(des, src, r);
		if (memcmp(des, src, r)) {
			printf("Error: random len test failed (r: %d) - mem_cpy_sse\n", r);
			ret |= 1;
		} else {
			putchar('.');
			fflush(0);
