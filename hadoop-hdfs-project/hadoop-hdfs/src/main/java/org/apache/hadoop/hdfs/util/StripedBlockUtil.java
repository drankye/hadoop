...
public class StripedBlockUtil {
  ...
  public static byte[][] initDecodeInputs(AlignedStripe alignedStripe,
    ...
    byte[][] decodeInputs =
        new byte[dataBlkNum + parityBlkNum][(int) alignedStripe.getLength()];
    for (int i = 0; i < alignedStripe.chunks.length; i++) {
      StripingChunk chunk = alignedStripe.chunks[i];
      if (chunk == null) {
        alignedStripe.chunks[i] = new StripingChunk(decodeInputs[i]);
        alignedStripe.chunks[i].offsetsInBuf.add(0);
        alignedStripe.chunks[i].lengthsInBuf.add((int) alignedStripe.getLength());
...
//              chunk.lengthsInBuf.get(j));
  ...
  public static AlignedStripe[] divideByteRangeIntoStripes (
    ...
    StripeRange ranges[] = new StripeRange[dataBlkNum];
    ranges[firstCell.idxInStripe] =
        new StripeRange(firstCellOffsetInBlk, firstCellSize);
    ...
    for (int i = firstCellIdxInBG; i <= lastCellIdxInBG; i++) {
      ...
      for (AlignedStripe s : stripes) {
        long stripeEnd = s.getOffsetInBlock() + s.getLength() - 1;
   ...
   * represents a {@link StripeRange} that is aligned with both the byte range
   ...
   * |cell|  +----+          |  |----|  |----|      doesn't end at last block
   * |    |                  |  |    |  |    | <- AlignedStripe4:
  ...
  public static class AlignedStripe {
    ...
    public AlignedStripe(long offsetInBlock, long length, int width) {
      ...
      this.range = new StripeRange(offsetInBlock, length);
      this.chunks = new StripingChunk[width];
    }

    public AlignedStripe(StripeRange range, int width) {
      this.range = range;
      this.chunks = new StripingChunk[width];
    }
    ...
    public String toString() {
      return "Offset=" + range.offsetInBlock + ", length=" + range.length +
   ...
   * starting at {@link offsetInBlock} and lasting for {@link length} bytes.
   * Note that StripeRange doesn't necessarily align with {@link StripingCell}.
   *
   * |<- Striped Block Group ->|
   *  blk_0     blk_1     blk_2
   *    |         |         |
   ...
   * |     |   |     |   |     |
   * |     |   |     |   |     |
   * |~~~~~|   |~~~~~|   |~~~~~|<-- {@link offsetInBlock} + {@link length} - 1
   * |     |   +-----+   +-----+
   * |     |
   * +-----+
   */
