...
/**
 * This is a slightly adapted version based on the same file in Intel ISA-L
 * library. It is copied here to test and verify the basic functions of ISA-L
 * integration. Note it's not serving as any sample to write an erasure coder
 * using the library, and a sample is to be written and provided separately.
 */
...
int main(int argc, char *argv[])
{
  ...
  h_gf_gen_rs_matrix(encode_matrix, m, k);
  ...
  // The matrix generated by gf_gen_cauchy_matrix
  ...
  for (rows = 1; rows <= 16; rows++) {
    ...
    for (size = EFENCE_TEST_MIN_SIZE; size <= TEST_SIZE; size += align) {
      ...
      // Perform matrix dot_prod for EC encoding
      // using g_tbls from encode matrix a
      h_ec_encode_data(size, k, m - k, g_tbls, efence_buffs, &efence_buffs[k]);
      ...
      // Recover data
      h_ec_init_tables(k, nerrs, decode_matrix, g_tbls);
      h_ec_encode_data(size, k, nerrs, g_tbls, recov, &temp_buffs[k]);
  ...
  for (rtest = 0; rtest < RANDOMS; rtest++) {
    ...
    // Perform matrix dot_prod for EC encoding
    // using g_tbls from encode matrix a
    h_ec_encode_data(size, k, m - k, g_tbls, ubuffs, &ubuffs[k]);
    ...
    // Recover data
    h_ec_init_tables(k, nerrs, decode_matrix, g_tbls);
    h_ec_encode_data(size, k, nerrs, g_tbls, recov, &temp_ubuffs[k]);
  ...
  for (size = TEST_LEN; size > 0; size -= align) {
    ...
    // Perform matrix dot_prod for EC encoding
    // using g_tbls from encode matrix a
    h_ec_encode_data(size, k, m - k, g_tbls, buffs, &buffs[k]);
