 ...
 */
package org.apache.hadoop.hdfs.ec.help;
...
public class GaloisField {
	 ...
	 /* int[] gen = {1};
	    ...
	    for (int i = 0; i < 3; i++) {
	      ...
	      for(int j=0;j<gen.length;j++){
			  System.out.println("gen["+j+"]=="+gen[j]);
		  }
	    }
	  ...
	  /*for(int m=0;m<gf.mulTable.length;m++){
		  for(int n=0;n<gf.mulTable[m].length;n++){
			  System.out.println("mulTable["+m+"]["+n+"]="+gf.mulTable[m][n]);
		  }
	  }
	  for(int m=0;m<gf.divTable.length;m++){
		  for(int n=0;n<gf.divTable[m].length;n++){
  ...
  /**
   * Given a Vandermonde matrix-fan de meng matrix- V[i][j]=x[j]^i and vector y, solve for z such
  ...
  public void remainder(int[] dividend, int[] divisor) {
    for (int i = dividend.length - divisor.length; i >= 0; i--) {
      ...
      // String aa = "div[dividend("+(i + divisor.length - 1)+")][divisor("+(divisor.length-1)+")]";
      //System.out.println(i+"-ratio=div[dividend("+(i + divisor.length - 1)+")][divisor("+(divisor.length-1)+")]");
