...
public class IsaReedSolomonDecoder extends org.apache.hadoop.hdfs.ec.rawcoder.impl.Decoder {
  ...
  private Random rand;
  private int bufSize;
  private byte[][] readBufs;
  private byte[][] writeBufs;
  ...
  public native static int init(int stripeSize, int paritySize);

  public native static int decode(ByteBuffer[] alldata, int[] erasured, int blockSize);
  ...
  void writeFixedBlock(
    ...
    for (long written = 0; skipBytes + written < limit; ) {
      ...
      start = System.nanoTime();
      decode(readByteBuf, erasedLocations, bufSize);
      end = System.nanoTime();
      calTotal += end - start;
